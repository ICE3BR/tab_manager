...
- `.duplicate-mode-bar`/`#sessionBar`/`#settingsBar`/`#bulkActionsBar` visibility is toggled via the `hidden` attribute in JS, not a CSS class — if you add `display` to a rule matching those elements, add a `[hidden] { display: none }` override too, or the attribute gets silently overridden (this exact bug happened once; see git history).
